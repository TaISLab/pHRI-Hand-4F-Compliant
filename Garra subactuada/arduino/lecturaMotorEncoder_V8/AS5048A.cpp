...
/*
 * Read angle register from n sensors conected in daisy chain
 * Takes the address of the register as a 16 bit word
 * Returns the value of the registers
 */
void AS5048A::read_daisyChain(int n_slaves){
  word command = 0b0100000000000000; // PAR=0 R/W=R
...
#ifdef AS5048A_DEBUG
  Serial.print("Read (0x");
  Serial.print(AS5048A_ANGLE, HEX);
  Serial.print(") with command: 0b");
  Serial.println(command, BIN);
#endif
  ...
  for(int i=0; i<n_slaves; i++){
    left_byte = SPI.transfer(0x00);
    right_byte = SPI.transfer(0x00);
    word lec = ( ((left_byte & 0xFF)<< 8) | (right_byte & 0xFF) ) & ~0xC000;
    daisyReadings[i] = (int)lec - (int)position;
    //if(daisyReadings[i] > 8191) daisyReadings[i] = -((0x3FFF)-daisyReadings[i]); //more than -180     
  }
  digitalWrite(_cs, HIGH);
